...
namespace curse_crypt
{
    ...
    class Program
    {
        ...
        static void FindMinMax(int fileLength, int registerLength = 4)   // Ділення файлу на max та min
        {
        ...
        static int ConfusionData(int code)  // сплутування даних
        {
        ...
        static void FileStream(int mode = 1)
        {
            ...
            uint j = 0;
            for (uint i = 0; i < firstStream.Length - 2; i++)
            {
                if (i < sizeHeader)
                {
            ...
            //using (MemoryStream memoryStream = new MemoryStream(dataImage, 0, dataImage.Length))
           // {
                //try
               // {
          ...
          //  }
            ...
            byte[] endAdress = { 255, 217 };
            MemoryStream memoryStream = new MemoryStream(dataImage, 0, dataImage.Length);
                    //memoryStream.Seek(0, SeekOrigin.Begin);
                    memoryStream.Write(dataHeader, 0, dataHeader.Length);
            memoryStream.Write(cryptBodyImage, 0, cryptBodyImage.Length);
                    //memoryStream.Write(endAdress, 0, endAdress.Length);
            Image image = Image.FromStream(memoryStream); //this error, can be  (memoryStream, false or true), only 1 stream can be here
                    image.Save(pathFirstFile, System.Drawing.Imaging.ImageFormat.Jpeg);
            //foreach (byte b1 in cryptBodyImage) memoryStream.WriteByte(b1);
            ...
            //image.Save(pathFirstFile, System.Drawing.Imaging.ImageFormat.Jpeg);
            //memoryStream.Close();
            //Image image1 = Image.FromStream(secondStream);
            ...
            //    secondStream.Write(endAdress, 0, endAdress.Length);
